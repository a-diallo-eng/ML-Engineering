...
if page == "🔍 Prediction":
    ...
    model_path = project_root / 'models' / 'best_heart_disease_model.pkl'
    
    if not model_path.exists():
        st.error("❌ Model not found. Please train the model first using ml_pipeline.py")
    else:
        try:
...
# ============================================================================
    ...
    2. **Preprocessing:** Data is standardized using the same scaler as the training data
    ...
    The model uses 13 clinical parameters:
    ...
    The model was trained on the UCI Heart Disease Dataset, a well-known dataset in
    the machine learning community for cardiovascular disease prediction.
    ...
    The application uses ensemble methods including:
    - Random Forest
    - Gradient Boosting
    - Logistic Regression
